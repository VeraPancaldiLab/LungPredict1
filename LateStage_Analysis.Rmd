...
```{r}
png(paste0(getwd(),"/Figures/Heatmaps/New2/DeconvolutionLATESTAGE.png"), width = 6000, height = 4500, res=250)
...
col = clinical.data[-which(clinical.data$response=="No data"),]
deconv_df2 = deconv_df[,colnames(deconv_df)%in%rownames(col)]
png(paste0(getwd(),"/Figures/Heatmaps/New2/DeconvolutionRESPONSE_Clusters.png"), width = 6000, height = 4500, res=250)
ha <- HeatmapAnnotation(Response = col$response, col = list(Response = c("Responders" = "green", "Non responders"= "red")))
Heatmap(t(scale(t(deconv_df2))), clustering_method_columns = "ward.D2", top_annotation = ha, width = unit(30, "cm"), height = unit(35, "cm"), column_dend_height = unit(5, "cm"), show_heatmap_legend = F)
dev.off()
