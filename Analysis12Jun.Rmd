...
```{r}
deconv <- as.matrix(read.delim("~/Downloads/all_deconvolutions_Counts_LPVan_TPM.txt", row.names=1))
...
res = random_forest(LP, colData_LP$cluster, 20)
...
png(paste0(getwd(),"/Figures/Heatmaps/New2/DeconvolutionLP_FS_JOINED.png"), width = 6000, height = 4500, res=250)
...
png(paste0(getwd(),"/Figures/Heatmaps/New2/DeconvolutionVan_FS_JOINED.png"), width = 6000, height = 4500, res=250)
...
hc1 = as.dendrogram(hclust(dist(scale(TFs)), method = "ward.D2"))
png(paste0(getwd(),"/DendogramTFs.png"), width = 6000, height = 4500, res=250)
plot(hc1)
rect.dendrogram(hc1, h=5,horiz=F)
dev.off()
x = correlation(t(TFs))
data = t(TFs)
...
plot(hc, horiz= T)
rect.dendrogram(hc, h=1,horiz=TRUE)
dev.off()
...
```{r}
MEs0_merge = MEs0_merge[,colnames(MEs0_merge)%in%rownames(dt)]
moduleTraitCor = cor(t(MEs0_merge), dt, use = "p")
