...
```{r}
deconv <- as.matrix(read.csv(paste0(getwd(), "/Input/Deconvolution/Deconvolution_Maha.csv"), row.names = 1, sep=","))
```
...
saveRDS(res, "Groups_cells_all_MAHA.rds")
saveRDS(groups, "Groups_cells_correlation_MAHA.rds")
saveRDS(groups_similarity, "Groups_cells_similarity_MAHA.rds")

write.csv(dt, "Deconvolution_after_MAHA.csv")
```
Input data
```{r}
Maha_TPM_normalization_bulk <- read.csv("~/LungPredict1/Input/Counts/Maha_TPM_normalization_bulk.txt", header=FALSE, row.names=1)
colnames(Maha_TPM_normalization_bulk) = Maha_TPM_normalization_bulk[1,]
Maha_TPM_normalization_bulk = Maha_TPM_normalization_bulk[-1,]
TFs = compute_TFs_scores(Maha_TPM_normalization_bulk, "Maha")
```

TFs filtering
```{r}
#Remove columns with low variance (no much change across samples)
TFs = TFs[-which(rowVars(TFs)<=summary(rowVars(TFs))[2]),]
...
png(paste0(getwd(),"/Figures/Heatmaps/Maha/TFs_clusters_MAHA.png"), width = 7500, height = 4500, res=250)
plot(hc)
...
plot(hc)
rect.dendrogram(hc, h = 5,horiz=F)
clusters2 <- cutree(hc, h = 5)
...
write.csv(TFs_modules_merge, "TFs_modules_MAHA.csv")
```

Correlating deconvolution with TFs activity
```{r}
idx = c("Cluster 10", "Cluster 11", "Cluster 8", "Cluster 9", "Cluster 5")
TFs_modules_merge = TFs_modules_merge[-which(rownames(TFs_modules_merge)%in%idx),]
...
png(paste0(getwd(),"/Figures/Heatmaps/Maha/Module_trait_relationships_Maha.png"), width = 4000, height = 1500, res=150)
...
png(paste0(getwd(),"/Figures/Heatmaps/Maha/Dendogram_Subgroups_cells_Maha.png"), width = 2500, height = 2500, res=150)
par(mar = c(5, 5, 4, 30)) #bottom, left, top, right
plot(hc, horiz= T)
rect.dendrogram(hc, h=0.6,horiz=TRUE)
dev.off()

clusters3 <- cutree(hc, h = 0.6)

png(paste0(getwd(),"/Figures/Heatmaps/Maha/Deconvolution_Maha.png"), width = 6000, height = 4500, res=250)
...
write.csv(t(deconv_df), "Deconvolution_after_MAHA_clusters.csv")

png(paste0(getwd(),"/Figures/Heatmaps/Maha/Deconvolution_Maha_NICHES.png"), width = 6000, height = 4500, res=250)
...
saveRDS(niches, "Cell_groups_Maha.rds")
```
Clustering patients 
```{r}
ClinicalData_Maha <- read.csv("~/LungPredict1/Input/ClinicalData/ClinicalData_Maha.csv", row.names=1)
data = t(deconv_df)
hc1 = as.dendrogram(hclust(dist(scale(data)), method = "ward.D2"))
png(paste0(getwd(),"/Figures/Heatmaps/New2/Patients_clusters_Maha.png"), width = 2500, height = 1500, res=150)
plot(hc1)
rect.dendrogram(hc1, k=2,horiz=F)
dev.off()
clusters <- cutree(hc1, k=2)
ClinicalData_Maha$cluster = NULL
ClinicalData_Maha$cluster = clusters
...
res = random_forest(t(deconv_df), ClinicalData_Maha$cluster, 10)
...
png(paste0(getwd(),"/Figures/Heatmaps/Maha/Deconvolution_Maha_FS_CLUSTERS_1_2.png"), width = 6000, height = 4500, res=250)
ha <- HeatmapAnnotation(Cluster = ClinicalData_Maha$cluster, Response = ClinicalData_Maha$Response, col = list(Cluster = c("1" = "yellow", "2" = "purple"), Response = c("Responders"="green", "Non responders" = "red")))
Heatmap(t(scale(res[[1]][,vec])), cluster_rows = F, column_dend_reorder = T,  clustering_method_columns = "ward.D2", top_annotation = ha, width = unit(30, "cm"), height = unit(20, "cm"), column_dend_height = unit(10, "cm"), row_dend_width = unit(10, "cm"))
dev.off()
```
...
res = random_forest(t(deconv_df), ClinicalData_Maha$Response, 10)


png(paste0(getwd(),"/Figures/Heatmaps/Maha/Clusters_Feature_Importance_Maha.png"), width = 2000, height = 1500, res=250)
ggplot(res[[2]], aes(x = reorder(rownames(res[[2]]), -Importance), y = Importance)) +
  geom_bar(stat = "identity", fill = "blue") +
  labs(x = "Features", y = "Importance") +
  theme(axis.text.x = element_text(angle = 90, hjust = 1))
...
png(paste0(getwd(),"/Figures/Heatmaps/Maha/Deconvolution_Maha_FS_CLUSTERS.png"), width = 6000, height = 4500, res=250)
ha <- HeatmapAnnotation(Response = ClinicalData_Maha$Response, col = list(Response = c("Responders" = "green", "Non responders" = "red")))
Heatmap(t(scale(t(deconv_df))), cluster_rows = F,  clustering_method_columns = "ward.D2", top_annotation = ha, width = unit(30, "cm"), height = unit(20, "cm"), column_dend_height = unit(10, "cm"), row_dend_width = unit(10, "cm"))
dev.off()
```

```{r}
RESPONSE <- read.csv("~/LungPredict1/Figures/Heatmaps/Maha/Anova_response_Maha.csv", row.names=1)
compute_violin(RESPONSE, ClinicalData_Maha$Response, "Maha")
```
