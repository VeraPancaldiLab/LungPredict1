...
```{r}
# Compute the position of labels
clinical.data2 <- clinical.data2[which(clinical.data2$diagnostic=="Adénocarcinome"),]
...
data <- count(clinical.data2$puncture_location) %>%
  mutate(prop = (freq/nrow(clinical.data2))*100) %>%
  arrange(desc(x)) %>%
  mutate(lab.ypos = cumsum(prop) - 0.5*prop) %>% 
...
ggplot(data, aes(x = "", y = round(prop), fill = fct_inorder(x))) +
  geom_col(width = 1, color = 1) +
  coord_polar("y", start = 0)+
  theme_void() +
  theme(legend.title = element_text(colour="black", size=10, face="bold"))+
  labs(title = "Number of patients with Adenocarcinoma - Puncture by location",
       fill = "Location") +
  scale_fill_manual(values = c("#0091D5", "#1C4E80", "#EA6A47"), breaks=c('extra-thoracique', 'Poumon', 'NA'), labels=c('Extra-thoracic','Lung', 'NA')) +
  geom_label_repel(data = data,
...
```{r}
# Compute the position of labels
data <- count(clinical.data2$type_of_puncture) %>%
  mutate(prop = (freq/nrow(clinical.data2))*100) %>%
  arrange(desc(x)) %>%
  mutate(lab.ypos = cumsum(prop) - 0.5*prop) %>% 
  mutate(csum = rev(cumsum(rev(round(prop)))), 
...
ggplot(data, aes(x = 2, y = round(prop), fill = fct_inorder(x))) +
  geom_bar(stat = "identity", color = "white") +
  coord_polar("y", start = 0)+
  theme_void() +
  theme(legend.title = element_text(colour="black", size=10, face="bold"))+
  labs(title = "Number of patients with Adenocarcinoma - Type of Puncture",
       fill = "Type of Puncture") +
  scale_fill_manual(values = c("#0091D5", "#1C4E80", "#EA6A47"),breaks = c('primaire', 'métastase', ''), labels=c('Primary', 'Metastasis', 'NA')) +
  geom_label_repel(data = data,
                   aes(y = pos, label = freq),
                   size = 4, nudge_x = 1, show.legend = FALSE)
```
