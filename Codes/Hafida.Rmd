...
```{r}
deconv <- as.matrix(read.delim("~/Downloads/deconvolutions_data2_159_first.txt", row.names=1))
deconv <- as.matrix(read.csv("~/Downloads/SCCHN.csv", row.names=1))
```
...
```{r}
file_name = "SCCHN"
write.csv(dt, paste0(getwd(),"/Deconvolution_after_", file_name, ".csv"))
saveRDS(res, paste0(getwd(),"/Groups_cells_all_", file_name, ".rds"))
saveRDS(groups, paste0(getwd(),"/Groups_cells_", file_name, ".rds"))
saveRDS(groups_similarity, paste0(getwd(),"/Groups_cells_similarity_", file_name, ".rds"))
```
