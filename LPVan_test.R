...
deconv <- read.delim("~/Downloads/all_deconvolutions_TPM_LUNGPREDICT.txt", row.names=1)
deconv <- read.delim("~/Downloads/all_deconvolutions_TPM_VANDERBILT.txt", row.names=1)
#Input file
...
clinical.data = clinical.data[-which(clinical.data$Batch==2),]
...
idx = c('R4163_YZ_8', 'R4163_YZ_18', 'R4163_YZ_28')
dt = dt[-which(rownames(dt)%in%idx),]
idx = c('R3388_YZ_8','R3388_YZ_28')
idx = c('R3388_YZ_48','R3388_YZ_12')
deconv = deconv[-which(rownames(deconv)%in%idx),]
clinical.data = clinical.data[rownames(clinical.data)%in%rownames(deconv),]
...
png(paste0(getwd(),"/Figures/Heatmaps/New/DeconvolutionLP.png"), width = 6000, height = 4500, res=250)
...
LP = dt[rownames(dt)%in%rownames(colData_LP),]
...
res = random_forest(LP, colData_LP$cluster, 30)
...
png(paste0(getwd(),"/Figures/Heatmaps/New/DeconvolutionLP_FS_JOINED.png"), width = 6000, height = 4500, res=250)
...
Van = dt[rownames(dt)%in%rownames(colData_Van),]
Van = Van[,colnames(Van)%in%colnames(res[[1]])]
colData_Van = colData_Van[rownames(colData_Van)%in%rownames(Van),]
png(paste0(getwd(),"/Figures/Heatmaps/New/DeconvolutionVan_FS_JOINED.png"), width = 6000, height = 4500, res=250)
Heatmap(t(scale(Van)), clustering_method_columns = "ward.D2", width = unit(30, "cm"), height = unit(20, "cm"), column_dend_height = unit(10, "cm"))
dev.off()

colData_Van2 = clinical.data[rownames(clinical.data)%in%rownames(colData_Van),]
ha <- HeatmapAnnotation(Canary = colData_Van2$CANARY,
                        Age = colData_Van2$Age_Bracket,
                        Gender = colData_Van2$Gender,
                        Race = colData_Van2$Race, 
                        BMI_Bracket = colData_Van2$BMI_Bracket,
                        Smoking= colData_Van2$Smoking_Status,
                        Prior_Cancer= colData_Van2$Prior_Cancer, 
                        Stages = colData_Van2$Stages_simplified)

png(paste0(getwd(),"/Figures/Heatmaps/New/DeconvolutionVan_FS_JOINED_annot.png"), width = 6000, height = 4500, res=250)
Heatmap(t(scale(Van)), clustering_method_columns = "ward.D2",  top_annotation = ha, width = unit(30, "cm"), height = unit(20, "cm"), column_dend_height = unit(10, "cm"))
dev.off()

Mutations_Vanderbilt <- read.csv("~/LungPredict/Input/ClinicalData/Mutations_Vanderbilt.csv", row.names=1)
clinical.data = clinical.data[clinical.data$pt_ID%in%rownames(Mutations_Vanderbilt),]
Mutations_Vanderbilt = Mutations_Vanderbilt[,c("KRAS", "EGFR")]
clinical.data = clinical.data[rownames(clinical.data)%in%rownames(Mutations_Vanderbilt),]
Mutations = Mutations_Vanderbilt %>%
  rownames_to_column("pt_ID") %>%
  mutate(pt_ID = as.integer(pt_ID)) %>%
  inner_join(clinical.data, by = "pt_ID")
rownames(Mutations) = rownames(clinical.data)
Van_2 = Van[rownames(Van)%in%rownames(Mutations),]
Mutations = Mutations[rownames(Mutations)%in%rownames(Van),]
colData_Van2 = Mutations
...
ha <- HeatmapAnnotation(Canary = colData_Van2$CANARY,
                        ...
                        Prior_Cancer= colData_Van2$Prior_Cancer, 
                        Stages = colData_Van2$Stages_simplified,
                        KRAS = colData_Van2$KRAS,
                        EGFR = colData_Van2$EGFR)

png(paste0(getwd(),"/Figures/Heatmaps/New/DeconvolutionVan_FS_JOINED_annot.png"), width = 6000, height = 4500, res=250)
Heatmap(t(scale(Van_2)), clustering_method_columns = "ward.D2",  top_annotation = ha, width = unit(30, "cm"), height = unit(20, "cm"), column_dend_height = unit(10, "cm"))
dev.off()
####################################################################End of deconvolution analysis

#Patients clustering 
clinical.data = read.csv("~/LungPredict/ClinicalLPVan.csv", row.names=1)
clinical.data = clinical.data[rownames(clinical.data)%in%rownames(dt),]
...
target = clinical.data$cluster
target = clinical.data[which(clinical.data$cluster%in%c(2,5)),]
df = dt[rownames(dt)%in%rownames(target),]
#Feature selection
random_forest <- function(data, target, k_features) {
  # Step 1: Random Forest Feature Selection
  target = as.factor(target)
...
res = random_forest(dt, clinical.data$cluster, 20)
...
png(paste0(getwd(),"/Figures/Heatmaps/New/DeconvolutionLP_FS.png"), width = 6000, height = 4500, res=250)
ha <- HeatmapAnnotation(Cluster = clinical.data$cluster,     
                        col = list(Cluster = c("1" = "green", "2" = "purple")))
Heatmap(t(scale(res[[1]])), clustering_method_columns = "ward.D2",  top_annotation = ha, width = unit(30, "cm"), height = unit(20, "cm"), column_dend_height = unit(10, "cm"))
dev.off()
...
png(paste0(getwd(),"/Figures/Heatmaps/Volcano_DEG_Cluster_Van.png"), width = 3000, height = 2000, res=200)
...
png(paste0(getwd(),"/Figures/Heatmaps/Heatmap_TFs_Van.png"), width = 6000, height = 4500, res=250)
ha <- HeatmapAnnotation(Cluster = clinical.data$cluster, col = list(Cluster = c("1" = "green", "2" = "purple")))
...
##########################################
clinical.data$cluster = as.factor(clinical.data$cluster)
p <- pca(t(scale(res[[1]])), metadata = clinical.data, removeVar = 0.1)
...
png(paste0(getwd(),"/Figures/Heatmaps/Biplot_deconvolutionLPVan.png"), width = 2000, height = 1500, res=200)
