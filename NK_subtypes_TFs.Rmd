...
idy = c("DeconRNASeq_Fig2ab.NSCLC_PBMCs_scRNAseq_sigmatrix_NK.cells","DeconRNASeq_scRNA.Seq_melanoma_Tirosh_sigmatrix_SuppFig_3.b_NK.cells", "DeconRNASeq_TIL10_NK.cells",
"Epidish_LM22_NK.cells.resting","CBSX__LM22__NK_cells_resting")
...
x = c()
for (i in 1:length(pt_ID)) {
  if((pt_ID[i]%in%pt_ID2)==T){
    x = c(x, pt_ID[i])
  }
}
pt_ID = pt_ID[-which(pt_ID%in%x)]
pt_ID2 = pt_ID2[-which(pt_ID2%in%x)]

clinical.data$NK = "na"
clinical.data$NK[rownames(clinical.data)%in%pt_ID] = "NK_cluster_1"
...
compute_msVIPER_scores = function(RNA.counts.normalized, test_name, ref_name, measure){
  ...
  # Plot DiffActive TFs
  png(paste0(getwd(),"/Figures/Heatmaps/msVIPER/NKs.png"), width = 1000, height = 800, res=100)
  print(plot(mra, mrs=15, cex=1, include = c("expression","activity")))
  dev.off()
  
}

compute_msVIPER_scores(Counts, "NK_cluster_1", "NK_cluster_2", clinical.data$NK)
```
...
idx = c("Quantiseq_NK_cell","DeconRNASeq_BPRNACanProMet_NK")
idy = c("DeconRNASeq_scRNA.Seq_melanoma_Tirosh_sigmatrix_SuppFig_3.b_NK.cells", "CBSX__LM22__NK_cells_activated","DeconRNASeq_LM22_NK.cells.resting")
...
compute_msVIPER_scores(Counts, "NK_cluster_1", "NK_cluster_2", clinical.data$NK, "LateStage2")
```
