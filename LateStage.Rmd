...
```{r}
compute_subgroups = function(data, file_name, thres_similarity = 0.01, thres_corr = 0.65, thres_change = 0.01){
...
write.csv(dt, paste0(getwd(),"/Figures/Heatmaps/New3/Deconvolution_after_LATE_STAGE.csv"))

saveRDS(res, paste0(getwd(),"/Figures/Heatmaps/New3/Groups_cells_all_LATESTAGE.rds"))
saveRDS(groups, paste0(getwd(),"/Figures/Heatmaps/New3/Groups_cells_correlation_LATESTAGE.rds"))
saveRDS(groups_similarity, paste0(getwd(),"/Figures/Heatmaps/New3/Groups_cells_similarity_LATESTAGE.rds"))
```
...
write.csv(TFs_modules_merge,  paste0(getwd(),"/Figures/Heatmaps/New3/TFs_modules_LATE_STAGE.csv"))
```
...
write.csv(t(deconv_df),  paste0(getwd(),"/Figures/Heatmaps/New3/Deconvolution_after_LATESTAGE_clusters.csv"))
...
saveRDS(niches,  paste0(getwd(),"/Figures/Heatmaps/New3/Cell_groups_LATESTAGE.rds"))
```
Random Forest
```{r}
...
# data = data[rownames(data)%in%rownames(clinical.data),]
# write.csv(clinical.data, "colData_response.csv")
# write.csv(data, "Data_response.csv")
...
Heatmap(t(scale(dat[,vec])),  column_split = clinical.data$cluster,  cluster_rows = F, clustering_method_columns = "ward.D2", top_annotation = ha, width = unit(30, "cm"), height = unit(35, "cm"), column_dend_height = unit(5, "cm"), show_heatmap_legend = F)
dev.off()

png(paste0(getwd(),"/Figures/Heatmaps/New3/Deconvolution_LATESTAGE_FS_CLUSTERS_RESPONSE.png"), width = 6000, height = 3500, res=250)
...
png(paste0(getwd(),"/Figures/Heatmaps/New3/Deconvolution_LATESTAGE_NICHES_RESPONSE.png"), width = 6000, height = 4500, res=250)
...
```{r}
RESPONSE <- read.csv("~/LungPredict1/Figures/Heatmaps/New3/Anova_response_LPIP.csv", row.names=1)
compute_violin(RESPONSE, clinical.data$response, "Response_LPIP")
```
